...
if [ -e $ROMFILE ]; then
	...
	CLOVER_OPTS+=(-device vfio-pci,bus=pcie-port-1,x-vga=on,host=$GFXPCI.0,multifunction=on,romfile=$ROMFILE,addr=0.0 )
#	CLOVER_OPTS+=(-device vfio-pci,host=$GFXPCI.0,multifunction=on,romfile=$ROMFILE,x-vga=on,rombar=0,addr=03.0 )
	...
	CLOVER_OPTS+=(-device vfio-pci,bus=pcie-port-1,x-vga=on,host=$GFXPCI.0,multifunction=on,addr=0.0 )
