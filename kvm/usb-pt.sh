...
-device vfio-pci,host=$USBPCI.0,bus=pcie.4,addr=0.0
)


if [ "x$USBPCI_1" != "x" ]; then
QEMU_OPTS+=(
-device vfio-pci,host=$USBPCI_1.0,bus=pcie.4,addr=1.0
)
./vfio-bind 0000:${USBPCI_1}.0
fi
if [ "x$USBPCI_2" != "x" ]; then
QEMU_OPTS+=(
-device vfio-pci,host=$USBPCI_2.0,bus=pcie.4,addr=2.0
)
